...
STT_STEP_PROMPTS: dict[str, str] = {
    # STT prompt는 빈 문자열로 — 어떤 값이든 whisper가 노이즈를 그 값으로 hallucinate함
    # address 단계만 예외적으로 최소한의 힌트 사용 (지명 인식률 향상)
    "default": "",
    "address_sido": "",
    "address_sigungu": "",
    "type": "",
...
2) 진행 중 다른 발급 가능한 서류로 변경 요청한 경우:
   - 먼저 "정말 변경하시겠습니까?"라고 확인 질문을 하세요.
   - 반드시 사용자의 음성 응답("네" 또는 "아니요")을 기다린 후에만 행동하세요.
   - 사용자가 "네" 또는 긍정 응답을 직접 말한 경우에만 → reset_workflow(reason: "서류 변경")를 호출하세요.
   - ⚠️ 절대로 확인 질문을 하자마자 바로 reset_workflow를 호출하지 마세요. 반드시 사용자 응답을 기다리세요.

3) 사용자가 절차를 취소하거나 다시 시작하고 싶어하는 경우:
   - 먼저 "정말 다시 시작하시겠습니까?"라고 확인 질문을 하세요.
   - 반드시 사용자의 음성 응답("네" 또는 "아니요")을 기다린 후에만 행동하세요.
   - 사용자가 "네" 또는 긍정 응답을 직접 말한 경우에만 → reset_workflow(reason: "사용자 취소")를 호출하세요.
   - ⚠️ 절대로 확인 질문을 하자마자 바로 reset_workflow를 호출하지 마세요. 반드시 사용자 응답을 기다리세요.
