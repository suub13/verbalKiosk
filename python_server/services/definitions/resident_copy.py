...
#   - "본인인증을 위해 화면에 이름, 생년월일, 주민번호 뒷자리 1자리, 통신사, 휴대폰 번호를 입력해 주세요."라고 안내하세요.
#   - 화면 폼에서 사용자가 정보 입력 → 인증번호 SMS → 인증번호 확인까지 자동으로 처리됩니다.
#   - 시스템 메시지로 identity_verified: true 가 오면 본인인증 완료입니다. 그 전에는 절대로 다음 단계로 넘어가지 마세요.
#   - ⚠️ 본인인증 대기 중에는 "도움이 필요하시면 말씀해 주세요" 같은 말을 절대 하지 마세요. 마이크가 꺼져 있어 음성인식이 불가능합니다. 그냥 조용히 기다리세요.
...
# 5단계 - 발급:
#   - 모든 절차 완료 후 issue_document 함수를 호출하세요.
...
5단계 - 전자서명 (기본발급/선택발급 모두 필수):
  ...
  - 전자서명 완료 시스템 메시지가 올 때까지 기다리세요. 완료 전까지 어떤 function call도 호출하지 마세요.
