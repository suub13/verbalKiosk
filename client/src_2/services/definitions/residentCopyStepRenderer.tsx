...
export function MicHelpButton() {
  ...
  // 수정 (static import 사용, WorkflowPanel 작동 버전과 동일한 순서)
const handleHelpPress = React.useCallback(async () => {
    if (micOpen) return;
    setMicOpen(true);
    setCountdown(10);                        // ← 수정
    pipelineBridge.sendMicUnblock?.();       // ← static import, 동기 호출
    pipelineBridge.startStreaming?.();       // ← static import, 동기 호출
    useStore.getState().setMuted(false);     // ← static import 사용
    ...
    const tick = () => {
      ...
      if (secs <= 0) {
        setMicOpen(false);
        useStore.getState().setMuted(true);  // ← dynamic import 제거
