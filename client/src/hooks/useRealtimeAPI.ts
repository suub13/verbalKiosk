...
export function useRealtimeAPI() {
  ...
  useEffect(() => {
    ...
    function setPendingCorrection(corrected: string) {
      ...
      // transcriptReceivedRef는 리셋하지 않음 — 이미 도착한 transcript가 있으면 그대로 유지
    ...
    const callbacks: RealtimePipelineCallbacks = {
      ...
      /** AI 오디오 청크 수신: speaking 상태로 전환 + 첫 delta에서 버블 즉시 표시 */
      onAudioDelta: () => {
        const currentState = useStore.getState().voiceState;
        if (currentState !== 'speaking') {
          transition('speaking');
          // 첫 오디오 delta = 음성 재생 시작 시점 → 버블을 여기서 표시
      ...
      onAudioDone: () => {
        // 오디오 전송 완료 시점에 AI 버블 표시 — 음성 재생 시작과 거의 동시
        if (transcriptReceivedRef.current) {
          const done = aiTranscriptDoneRef.current;
          if (done !== null) {
            aiTranscriptDoneRef.current = null;
      ...
      onTranscriptDone: (transcript) => {
        // 항상 버퍼에 저장 → onAudioDone 또는 flushAiTranscript에서 표시
        aiTranscriptDoneRef.current = transcript;
        if (transcriptReceivedRef.current) {
          // 유저 transcript 이미 도착 & 오디오도 끝났을 수 있으니 즉시 flush 시도
          // (onAudioDone이 먼저 왔다면 이미 flush됨, 아니면 여기서 flush)
          const done = aiTranscriptDoneRef.current;
          if (done !== null) {
            aiTranscriptDoneRef.current = null;
            addMessage('assistant', done, true);
