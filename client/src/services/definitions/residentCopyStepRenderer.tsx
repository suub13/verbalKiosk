...
export function MicHelpButton() {
  ...
  const portalTarget = React.useMemo(() => {
    ...
    return document.getElementById('mic-portal-root') || document.body;
  }, []);
    ...
    >
      <button
        ...
        style={{
          ...
          height: 68,
          borderRadius: '50%',
          border: micOpen ? '2px solid #3b82f6' : '2px solid rgba(255,255,255,0.9)',
          cursor: micOpen ? 'default' : 'pointer',
          background: micOpen ? '#3b82f6' : 'rgba(255,255,255,0.40)',
      ...
      <span
        style={{
          ...
          fontWeight: 600,
          textAlign: 'center',
          color: micOpen ? '#3b82f6' : '#3b82f6',
          textShadow: micOpen ? 'none' : '0 1px 4px rgba(0,0,0,0.3)',
...
export function SignDetail({ data }: { data: ResidentCopyData }) {
  ...
  if (json.success) {
    ...
    // 완료 화면 5초 유지 후 → AI 신호 전송 + 미리보기 이동 동시 처리
    setTimeout(async () => {
      // AI 파이프라인 신호 (이동 직전에 보내야 화면이 유지됨)
      const { pipelineBridge } = await import('@/services/pipelineBridge');
      ...
      <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 24px', gap: 20, height: '100%' }}>
        ...
        {errorMsg && (
          <div style={{
            ...
            <span style={{ fontSize: 17, fontWeight: 700, color: '#fff', textAlign: 'center' as const }}>
              전자증명서 발급 오류가 발생했습니다
            </span>
            <span style={{ fontSize: 14, color: 'rgba(255,255,255,0.85)', textAlign: 'center' as const, lineHeight: 1.6 }}>
              {errorMsg}
            </span>
          </div>
        )}
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {errorMsg && (
        <div style={{
          marginTop: 16, padding: '18px 22px', borderRadius: 16,
          background: '#DC2626',
          boxShadow: '0 4px 20px rgba(220,38,38,0.35)',
