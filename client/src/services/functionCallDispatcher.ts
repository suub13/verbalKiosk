  ...
  if (name === 'navigate_step') {
    try {
      ...
      const stepGuidance: Record<string, string> = {
        verify: '화면이 본인인증 단계로 전환되었습니다. 화면에 본인인증 폼이 표시되었습니다. 사용자에게 "이름, 생년월일, 주민번호 뒷자리 한 자리, 통신사, 휴대폰 번호를 화면에 입력해 주세요"라고 안내하세요. 사용자가 인증번호를 받아 인증까지 완료하면 시스템 메시지가 전달됩니다. 완료 전까지 다음 function call을 호출하지 마세요.',
